...
Vadym Dudarenko  Ivan Grakhovski  Ismayil Ismayilov
...
We create an empty list to store tem names, then initialize an index for team names. After that we print the dictionary of football team names.
```{r}
team_name <- list()
index <- 0
...
*Print the number of World Cup championships by country*
```{r}
...
*Set the initual values of "Home_Team_Champions" and "Away_Team_Champions" columns in the "matches" dataset to 0 *
```{r}
...
*Create a new column named "Winner" in the "matches" dataset and initialize all values as "-", then define function "calculate winner" to determine the winner based on goal scores. After that update "matches" dataset with winnerinformation.*
```{r}
matches$Winner <- "-"
...
*Define function "replace_team_name" to replace team names in "matches" dataset. Update "matches" dataset with replaced team names".*
```{r}
...
*Remove unnecessary columns from "matches" dataset. Replace NA values with 0.*
```{r}
...
test <- matches[!sample, ]  
```
*Build an SVM model for classification using the training data*
```{r}
...
*Make predictions in the test data using the trained data. Print the predicted values*
```{r}
pred_test <- predict(model,newdata = test)
pred_test
```
*Make predictions on the training data using the train model and store the results, then calculate the accuracy of the model on the training data by comparing the predictions with the actual values.*
```{r}
pred_train1 <- predict(model, train)
mean(pred_train1 == train$Winner)
```

*Make predictions on the test data using the trained model and store the results in "pred_test", then calculate the accuracy of the model on the training data by comparing the predictions with the actual values (test$Winner).*
```{r}
pred_test <- predict(model, test)
mean(pred_test == test$Winner)
```

*Create a data frame named results containing the actual and predicted values from the test data.*
```{r}
...
*Print the classification report, which includes evaluation metrics calculated using the confusion matrix.*
```{r}
print(cm)
```
